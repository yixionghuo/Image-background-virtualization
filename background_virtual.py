...
def virtual_background(original_img, mask):
    re = transforms.Resize(520)
    img = re(original_img)

    mask = PIL2cv(mask)
    img = PIL2cv(img)

    img_virtual = cv2.blur(img, (5, 5))
    mask = cv2.blur(mask, (2, 2))

    result = np.zeros_like(img_virtual)
    result[mask > 0] = img[mask > 0]
    result[mask == 0] = img_virtual[mask == 0]
...
def main():
    ...
    # create model
    t_start_model = time_synchronized()
    ...
    t_end_model = time_synchronized()
    print("loading model time: {}".format(t_end_model - t_start_model))
